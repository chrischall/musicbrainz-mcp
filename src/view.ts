 ...
 *
 * The read tools in this server hand back MusicBrainz's payload close to
 ...
 * honestly say which of MusicBrainz's fields matter and which are noise.
 *
 ...
 */
export const MB_VIEWS = ['compact', 'full'] as const;

const NOTE =
  'compact strips image/avatar URLs from the response; "full" returns MusicBrainz\'s payload untouched. ' +
  'No field projection: this server has no verified record of which MusicBrainz fields matter, and inventing ' +
  'one would risk dropping a field a caller needs.';

/** The `view` parameter every read tool in this server takes. */
export const viewArg = (): ReturnType<typeof viewParam> => viewParam(MB_VIEWS, { note: NOTE });
...
export function viewResponse(view: string | undefined, data: unknown): ReturnType<typeof minifiedResult> {
  const rung: View = resolveView(view, MB_VIEWS);
