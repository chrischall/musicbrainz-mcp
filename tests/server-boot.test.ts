...
function listToolsViaStdio(entry: string, cwd: string): Promise<string[]> {
  return new Promise((resolve, reject) => {
    const child = spawn('node', [entry], {
      cwd,
      // No OAuth creds: the server must still boot and serve tools/list (reads
      // need no auth; the write-config error is deferred to the first write).
      env: { ...process.env, MUSICBRAINZ_OAUTH_CLIENT_ID: '', MUSICBRAINZ_OAUTH_CLIENT_SECRET: '', MUSICBRAINZ_OAUTH_REFRESH_TOKEN: '' },
...
describe('server boot (built artifacts)', () => {
  it('bundled .mcpb (dist/bundle.js) boots WITHOUT node_modules and lists tools', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'musicbrainz-mcpb-'));
    try {
      ...
      expect(tools).toContain('musicbrainz_healthcheck');
    } finally {
